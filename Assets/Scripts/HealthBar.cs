...
public class HealthBar : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        /*
         * REPURPOSED FOR OVERCLOCK. Overclock and health are both out of 100
        ...
        */
        ...
        kite7.fillAmount = Mathf.Lerp(0f, 0.6f, Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.01415f) / 0.01415f);//kite7 is half of a full kite's percentage
        //kite7.color = healthColor;
        runningTotal += 0.01415f;//0.01415

        rect6.fillAmount = (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.1384f) - runningTotal) / 0.1384f;
        //rect6.color = healthColor;
        runningTotal += 0.1384f;//0.15255

        kite6.fillAmount = Mathf.Lerp(0f, 0.71f, (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.0283f) - runningTotal) / 0.0283f);
        //kite6.color = healthColor;
        runningTotal += 0.0283f;//0.18085

        rect5.fillAmount = (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.1384f) - runningTotal) / 0.1384f;
        //rect5.color = healthColor;
        runningTotal += 0.1384f;//.31925

        kite5.fillAmount = Mathf.Lerp(0f, 0.71f, (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.0283f) - runningTotal) / 0.0283f);
        //kite5.color = healthColor;
        runningTotal += 0.0283f;//0.34755

        rect4.fillAmount = (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.1384f) - runningTotal) / 0.1384f;
        //rect4.color = healthColor;
        runningTotal += 0.1384f;//0.48595

        kite4.fillAmount = Mathf.Lerp(0f, 0.71f, (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.0283f) - runningTotal) / 0.0283f);
        //kite4.color = healthColor;
        runningTotal += 0.0283f;//0.51425

        rect3.fillAmount = (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.1384f) - runningTotal) / 0.1384f;
        //rect3.color = healthColor;
        runningTotal += 0.1384f;//0.65265

        kite3.fillAmount = Mathf.Lerp(0f, 0.71f, (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.0283f) - runningTotal) / 0.0283f);
        //kite3.color = healthColor;
        runningTotal += 0.0283f;//0.68095

        rect2.fillAmount = (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.1384f) - runningTotal) / 0.1384f;
        //rect2.color = healthColor;
        runningTotal += 0.1384f;//0.81935

        kite2.fillAmount = Mathf.Lerp(0f, 0.71f, (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.0283f) - runningTotal) / 0.0283f);
        //kite2.color = healthColor;
        runningTotal += 0.0283f;//0.84765

        rect1.fillAmount = (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.1384f) - runningTotal) / 0.1384f;
        //rect1.color = healthColor;
        runningTotal += 0.1384f;//0.98605

        kite1.fillAmount = Mathf.Lerp(0f, 0.378f, (Mathf.Clamp(overclockCharge, runningTotal, runningTotal + 0.01415f) - runningTotal) / 0.01415f);
